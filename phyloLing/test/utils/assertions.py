...
def format_float(value: float,
                 places: int = decimal_places + 1) -> str:
    return f"{value:.{places}f}"


def format_elapsed_time(time: timedelta) -> str:
...
def _assert_distance_matrices_equal(test: unittest.TestCase,
    ...
    summary.align = 'r'
    summary.float_format = f".{decimal_places + 1}"
...
def _assert_tree_distances_equal(test: unittest.TestCase,
    ...
    max_difference = 10 ** -decimal_places
    formatted_first_value = format_float(first_value)
    formatted_second_value = format_float(second_value)
    formatted_delta = format_float(abs(first_value - second_value))

    test.assertAlmostEqual(
        first_value, second_value,
        decimal_places,
        f"{distance_label} tree distances are not equal: {formatted_first_value} != {formatted_second_value}, |Δ|: {formatted_delta}"
    )
    if math.isclose(first_value, second_value, rel_tol=max_difference):
        logger.info(f"{distance_label} tree distances are equal.")
...
def assert_determinism(test: unittest.TestCase,
    ...
    for i in range(determinism_test_iterations):
        ...
        logger.info(f"Running iteration {formatted_iteration} for {language_family}...")
        if last_iteration_time is not None:
            ...
            estimated_end_time = format_datetime(datetime.now() + estimated_remaining_time)
...
def assert_tree_distances_improved(test: unittest.TestCase,
    ...
    for reference_tree in result.reference_trees:
        ...
        best_tree_distance_formatted = format_float(best_tree_distance)
        result_tree_distance_formatted = format_float(result_tree_distance)
